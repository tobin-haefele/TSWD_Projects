...
In this last section I want to focus on the agencies/programs that refer individuals to the Housing Solutions Fund. The main question I had is if there is a significant difference between the amount of money an individual gets based off of their referring entity. Using a simple regression model to see if there is any statistical significance between the Referral forms. Looking at the results from that model, we can see that there appears to be a significant decrease in the amount of funds when referred from BLANK and BLANK agencies. Due to the small sample size this could be an outlier and would need further evaluation to determine if this is a significant finding.

```{r Form Stat, echo=FALSE, results='hide', message=FALSE}
#load data
solutions_fund <- X20231122_solutions_fund
...
# Calculate the average paid for each Referral from
average_paid_by_referral <- solutions_fund %>% 
  group_by(Referral.from) %>%
  summarize(AveragePaid = mean(Total.paid, na.rm = TRUE)) %>%
  arrange(desc(AveragePaid))

print(average_paid_by_referral)

#filter out rows where referral = MISSING
filtered_data <- solutions_fund %>%
  filter(Referral.from != "MISSING")

#create a regression model
model <- lm(Total.paid ~ Referral.from, data = filtered_data)
...
Looking at the basic statistics we can see that on average over 84% of the funding is going to those who are white. 
